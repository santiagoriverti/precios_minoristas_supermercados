...
# Antes se escribia un objeto Leaflet por producto x sucursal (~1,6 KB cada uno,
# con popup y tooltip propios). Con muchos productos el HTML se iba a decenas de
# MB y el 95% eran marcadores ocultos (solo se ve uno por vez). Ahora va SOLO
# el JSON de sucursales -que ya existia para los popups- mas lat/lon, y Leaflet
# dibuja al vuelo los del producto elegido. Ver BUG-31.
# ============================================================
...
function fmt(x){return "$"+Number(x).toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2});}
function mix(a,b,f){
...
function val(s,id){var c=s.can[id];return (c===undefined)?undefined:c;}
function popup(s,id){
 var c=s.can[id];
...
for _SFX, _TIT, _VCOL in [('','mediana','precio_producto'), ('_prom','promedio','precio_producto_prom')]:
    # ── Un registro por sucursal: metadata + lat/lon + valor de cada producto ────
    _popup_data = {}
    for _pid in _prods_con_datos:
        for _, _r in producto_geo_dict[_pid].iterrows():
            ...
            _popup_data[_sk]['can'][_pid] = round(float(_r[_VCOL]), 2)


    # Escala de color por producto (mismos percentiles que usaba LinearColormap)
    _items_cfg = []
    for _pid in _prods_con_datos:
        _v = producto_geo_dict[_pid][_VCOL]
    ...
    info_h = (f'<div id="info" style="position:fixed;top:10px;left:50px;width:340px;background:white;border:2px solid #0055A4;'
              ...
              f'<b>{len(_popup_data):,}</b> sucursales · <b>{len(_items_cfg)}</b> productos<br>'
    ...
    _tit_pag = f'Mapa de precios por supermercado — {NOMBRE_MES_TITLE}'
    _desc_pag = (f'{len(_popup_data):,} sucursales de todo el pais con el precio de 'f'{len(_items_cfg)} productos ({_TIT}). Datos del SEPA.')
