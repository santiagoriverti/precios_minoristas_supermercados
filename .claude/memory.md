...
**Más tarde todavía, misma sesión — bug real reportado por el usuario tras correr nb05 en Colab (⚠️ CORREGIDO):**

El usuario corrió nb05 con Coca-Cola Lata 354cc (EAN 7790895000232) y Fernet Branca 750ml (EAN 7790290101602) y vio caídas de precio ~100x en meses puntuales (ago-nov 2024 y jun-2026), rompiendo la serie. **Causa raíz confirmada leyendo el código (no solo por el patrón de datos):** la detección del factor centavos↔pesos (heurística: mediana de precios > 10.000 ⇒ centavos) se calculaba sobre la mediana de **solo los EANs que pidió el usuario**. Con canastas de 40-75 productos (nb02) esa mediana es robusta; con 1-2 productos (nb05) es frágil — el propio "ref_e" de respaldo que copié de nb02 quedaba **muerto**: el chunk del CSV ya se filtraba a `PRODUCTOS_EANS_NORM` ANTES de chequear si algún EAN de referencia estaba presente, así que `ref_d` siempre daba vacío y caía al mismo cálculo frágil. La CELDA 9 (lectura histórica, `_leer_mes_hist`) ni siquiera tenía ese resguardo — usaba la mediana cruda directo.

**Fix aplicado en `gen_nb05.py`:**
- `REF_EANS_FACTOR` (mismos 3 EANs de referencia que usa nb02) ahora se define una vez en la CELDA 2 (junto a `normalizar_ean`).
- CELDA 6 y CELDA 9 leen el CSV filtrando por `PRODUCTOS_EANS_NORM | REF_EANS_FACTOR` (unión), separan los precios crudos de los EANs de referencia en una muestra aparte (`muestra_factor`/`_muestra_ref`), y el factor se calcula sobre **esa** muestra (con fallback a la mediana de los productos propios si la referencia no aparece ese mes) — igual patrón que nb02, pero ahora efectivamente funciona.
- `datos`/`_agg` se siguen filtrando a solo los EANs del usuario antes de devolver, para no ensuciar conteos ni resultados con los EANs de referencia.

**Validado con un caso dirigido** (no solo el smoke test general): construí un mes sintético donde el EAN seguido reporta un precio crudo ambiguo (8.500, por debajo del umbral) en sus únicas 2 sucursales, y el EAN de referencia reporta un crudo claramente en centavos (450.000). Ejecuté el código real de la CELDA 9 contra ese escenario: **sin el fix da $8.500 (mal), con el fix da $85 (bien, ancla a la referencia)**. Volví a correr el smoke test end-to-end completo (20 celdas) después del fix — sigue pasando limpio, sin cambios de resultado (el dataset sintético original no tiene los EANs de referencia, así que ahí no había diferencia que mostrar).

**Pendiente:** el usuario tiene que volver a correr nb05 en Colab con Coca-Cola/Fernet para confirmar que la serie ya no cae abruptamente en esos meses.
