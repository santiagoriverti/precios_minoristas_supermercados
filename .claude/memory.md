...
- **✅ CORRIDO [2026-08-27] — éxito + fix de falsos positivos**. 1er run con name-based: cobertura ambos lados saltó (Dulces 1001→2534, Saladas 2319→2516), mapa 1377→2410 sucs / 19 cadenas (DIA capturado). PERO la regla de **dulces metió falsos positivos de ALTA cobertura**: "Mermelada Naranja Arcor" (ns=1181), "Yerba Mate Naranja" (ns=494), "Caramelos Frutilla Día" (ns=967), tabletas/bocaditos de chocolate — porque los sabores sueltos ('naranja','frutilla','chocolate') matcheaban bebidas/dulces no-galletita. El filtro de plausibilidad sacó los extremos (bebidas lácteas, alfajor, pimentón) pero no estos. → **dulces +97% y canasta ~148% del 1er run están CONTAMINADOS, no usar**. **✅ FIX [2026-08-27]**: la regla de dulces ahora **REQUIERE sustantivo de galletita** (galletit|galleta|biscuit|bizcoch|oblea|scon|pepa|magdalena|masita|vainilla|marmolad|…) + exclusiones ampliadas (yerba|mate|mermelada|caramelo|tableta|bocadito|bombon|jugo|bebida|agua|cacao|helado|alfajor|…). Validado: dulces 152→84 candidatos, 0 sospechosos; config 319→**250 EANs limpios**. Fideos/saladas/pan rallado ya estaban limpios (verificado en Detalle real). **✅ RE-CORRIDO Y LIMPIO [2026-08-27] — BASE CERRADA.** Config 250 EANs, cache `brecha_dia_a027dc57_v2`. Dulces 0 falsos positivos (verificado en Detalle). **NÚMEROS DEFINITIVOS (mediana, $/100g)**: dulces **+147,6%** · pan rallado +187,9% · saladas +190,8% · fideos +239,1% (rango coherente). Cobertura ambos lados: Fideos 2460 · **Dulces 2525** · Saladas 2516 · Pan rallado 792. Canasta +176% (ago), serie plana +6,3pp. Mapa: **19 cadenas, DIA 1001 sucursales** (la más grande) + Mariano Max. Filtro sacó solo 4 EANs (2 Matarazzo mal etiquetados + Smams error + 1 Schar). Detalle en `docs/BRECHA_CELIACA.md §12`. **➡️ MEDICIÓN/DATOS CERRADOS A NIVEL PAPER — LISTO PARA LA HEDÓNICA §3.9** (determinantes: tiempo/concentración/HHI/geografía/cadena con EF).
...
1. **nb06 — ✅ CORRIDO Y VALIDADO EN COLAB [2026-08-25, julio 2026].** EANs TACC curados (alta cobertura, canasta_representativa) + fix de RAM (colapso a mediana mensual, caché `_v2`). Resultado real: brecha por tipo +138% (dulces) → +362% (pan rallado); canasta acotada +240% (jul-26) desde +111% (ene-24). Números y cobertura en `docs/BRECHA_CELIACA.md` §12. **El usuario decidió DEJARLO ASÍ por ahora** (no re-correr, no tocar EANs/método). **🎓 Ahora el objetivo es un PAPER ACADÉMICO** sobre la brecha y sus determinantes (tiempo, concentración, geografía, cadena). **Próximo trabajo = revisión metodológica con rigor científico**: ver la **agenda en `docs/BRECHA_CELIACA.md` §13** (matching TACC↔sin-TACC, efecto tamaño de envase, sesgo de selección de sucursales, efecto composición entre cadenas, HHI, panel temporal). Ajuste fino menor conocido: 2 EANs sin presentación (`7790040133471`, `7798079230062`) entran sin normalizar en Galletitas dulces (cuesta otro re-read de ~70 min si se corrige).
...
3. **Caché**: al abrir en Colab se reconstruye solo (nb02/nb05: `hist_union_..._v2m.parquet`; nb06: `brecha_dia_..._v2.parquet`). Los `.parquet` viejos/huérfanos (incl. `brecha_dia_*_v1.parquet` y el hash TACC viejo) se pueden borrar de `output_*/_cache`.
...
- **MÉTODO VIGENTE = brecha INTRA-SUCURSAL por tipo, listas amplias, $/100g** (evolucionó en 3 runs reales): 1er run condición estricta (ambos lados misma sucursal mismo día ≥3 tipos) → 0 obs por baja cobertura sin-TACC; 2do intento pooled por grupo → daba +237% por presentaciones distintas y EANs espurios; **versión final** (lo que pidió el usuario): en cada sucursal, por tipo, se usa el precio $/100g de LOS candidatos que la sucursal tenga (mediana), y si tiene ambos lados se calcula la brecha del tipo INTRA-SUCURSAL. **Cambios**: (a) normalización `$/100g` (presentación del maestro → gramos, en CELDA 3; `EAN_GRAMS` CELDA 4; `precio_100` CELDA 7); (b) **listas amplias** de candidatos por tipo/lado en la CELDA 1 (fideos ~10 TACC incl. Matarazzo estándar, etc.; harina/premezcla COMENTADA por ser sustitución); (c) `MIN_TIPOS=1`. CELDA 7 arma `tp`(median $/100g por suc,día,tipo,rol) → `bt_dia` (merge tacc/sin, ambos presentes) → `brecha_suc_dia` (canasta). CELDA 8: **brecha_tipo** (output CLAVE) + `_mes`/`_prov`, series desde brecha_suc_dia. Hojas: Cobertura, Brecha_tipo/_mes/_prov, Serie_*, Brecha_provincia/cadena, Concentracion, Brecha_sucursal, Detalle_producto (con grams+precio_100g). **Hallazgo (NO bug)**: la brecha por producto TACC-sustituible es grande (galletitas ~+140/200%, pan rallado ~+320% en $/100g), muy por encima del 9% de la canasta completa (que "maquilla" con productos naturalmente sin gluten). Vetear por tipo con `Cobertura`/`n_sucursales`. **⚠️ 4º run real → método cambiado a INTRA-SUPER por (sucursal, MES)** (antes por-día daba n=1): `tp`=(suc,mes,tipo,rol) mediana $/100g → `bt_sm` (merge tacc/sin) → `brecha_suc_mes`; series solo MENSUAL (diaria/semanal vacías). **✅ RESUELTO [2026-08-25]**: (a) los EANs **TACC (regular)** nicho se reemplazaron por los de **alta cobertura** de `canasta_representativa_2026-07.xlsx` (hoja `Candidatos`, ~1900-2500 sucs; ver CELDA 1 actual del generador); (b) **fix de RAM**: la CELDA 6 ahora es LECTURA MENSUAL — colapsa los días a la **mediana mensual por (sucursal, EAN)** durante la lectura (incluso por-chunk), porque con EANs de alta cobertura el formato fila-por-día reventaba la RAM de Colab; caché bumpeado a `brecha_dia_*_v2.parquet` (esquema sin columna `fecha`); `datos_dia` ahora es suc×EAN×**mes**. **5º run OK** (julio 2026, sin OOM): resultados en `docs/BRECHA_CELIACA.md` §12. El usuario decidió dejar EANs/método así; el siguiente foco es la **revisión metodológica para el paper** (`docs/BRECHA_CELIACA.md` §13). Cambiar EANs vuelve a invalidar el caché (~70 min de re-read).
...
**Config (CELDA 1) — ESTADO ACTUAL [2026-08-25]**: dict `TIPOS = {tipo: {qty, tacc:[EANs], sin_tacc:[EANs]}}`, **4 tipos activos** (Harina/premezcla COMENTADO por ser sustitución, no dicotomía limpia). Lado **TACC curado por cobertura** (canasta_representativa, hoja `Candidatos`, ~1900-2500 sucs, excluyendo premium/snacks): Fideos secos (Lucchetti/Matarazzo/Favorita 500g), Galletitas dulces (9deOro/Don Satur/Chocolinas/Bagley/Sonrisas/Maná), Galletitas saladas (9deOro/Don Satur/Traviata/Tosti/Hogareñas), Pan rallado (Preferido/Mamá Cocina/Lucchetti/Pureza/Favorita). Lado **sin-TACC** sin cambios (listas de nicho de baja cobertura — el limitante intrínseco). `MIN_TIPOS=1`. La fuente de verdad de las listas es siempre `notebooks/gen_nb06.py` CELDA 1.
