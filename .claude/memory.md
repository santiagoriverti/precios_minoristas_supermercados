...
1. **nb06 — CORRER EN COLAB con los EANs TACC curados [HECHO 2026-08-25 la curación; falta el run].** Ya se reemplazaron las listas `tacc` de la CELDA 1 con los de alta cobertura de `canasta_representativa_2026-07.xlsx` (hoja `Candidatos`: filtré por subcategoría Pastas Secas / Galletitas Dulces+Saladas / Pan Rallado y elegí los de mayor `n_sucursales`, ~1900-2500, excluyendo premium/snacks que inflan `tacc_100`). El `.ipynb` está regenerado y el test sintético pasa. **⚠️ FIX RAM [2026-08-25]**: el primer intento de correr con los EANs nuevos **reventó la RAM de Colab** — como los EANs regulares tienen ~2500 sucursales, `datos_dia` en formato fila-por-día explotaba a decenas de millones de filas. **Solución**: la CELDA 6 ahora **colapsa a la mediana mensual por (sucursal, EAN) durante la lectura** (incluso por-chunk), reduciendo el footprint ~30× — sin perder nada porque el método agrega por MES. Caché bumpeado `brecha_dia_*_v1` → `_v2`. **Próximo: el usuario lo corre en Colab de nuevo** (re-read ~70 min por el cambio de EANs) y revisamos `Brecha_tipo` + hoja `Cobertura` (n_sucursales por tipo/lado). Si algún tipo sale con pocos supers del lado sin-TACC, se afina esa lista. El lado sin-TACC es el limitante intrínseco (productos celíacos = nicho).
