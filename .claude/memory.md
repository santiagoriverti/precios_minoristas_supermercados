...
- **Normalización $/100g + BRECHA POR TIPO** (⚠️ 2do run real: la canasta pooled daba +237%): se normaliza el precio a `$/100g` (presentación del maestro, columnas `producto_cantidad_presentacion`/unidad → gramos, en CELDA 3; `EAN_GRAMS` en CELDA 4; `precio_100` en CELDA 7) y se reporta la **brecha POR TIPO** (`brecha_tipo`/`_mes`/`_prov`, hojas nuevas + 2 gráficos) — **ese es el número clave**, no la canasta agregada. Hallazgo (NO bug): la brecha por producto TACC-sustituible es grande (galletitas ~+140/200%, pan rallado ~+320%), muy por encima del 9% de la canasta completa — el 9% "maquilla" porque promedia productos naturalmente sin gluten. Se vetea cada tipo con `n_tacc`/`n_sin` y `Cobertura`; los EANs de **fideos TACC** salieron espurios ($53/100g, 7 sucursales) y la **harina/premezcla** es sustitución (+719%) — el usuario debe reemplazar/ajustar esos EANs.
